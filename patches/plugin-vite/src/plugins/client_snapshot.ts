import { crawlFsItem, specToName } from "jsr:@fresh/core@^2.0.0/internal-dev";
import * as path from "jsr:@std/path@1";
import type { Plugin, ViteDevServer } from "npm:vite@^8.3.0";
  ...
  return [
    {
      ...
      load: {
        ...
        handler() {
          const imports = Array.from(islands.keys())
            .map((file, i) => {
              return `export const mod_${i} = await import(${JSON.stringify(
                file,
              )});`;
            })
            .join("\n");
