  ...
  return {
    ...
    async resolveId(id, importer, options) {
      ...
      if (!isHttp && path.isAbsolute(id)) {
        id = path.toFileUrl(path.normalize(id)).href;
      }
    ...
    async load(id) {
      ...
      if (meta.type === RequestedModuleType.Default && !JS_REG.test(id)) {
        return;
      }
...
function parseDenoSpecifier(spec: DenoSpecifier): {
  type: RequestedModuleType;
  specifier: string;
} {
...
function babelTransform(options: {
  ...
  if (!ssr && (id.endsWith(".tsx") || id.endsWith(".jsx"))) {
    presets.push([
      babelReact,
      {
        runtime: "automatic",
