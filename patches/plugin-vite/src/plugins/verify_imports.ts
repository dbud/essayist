import * as cl from "jsr:/@std/fmt@^1.0.7/colors";
import path from "node:path";
import type { PluginContext } from "npm:rolldown@^1.2.6";
    ...
    resolveId: {
      ...
      async handler(id, importer) {
        ...
        if (id.startsWith(".")) {
          ...
          if (resolved !== null) {
            const key = `${this.environment.config.consumer}::${resolved.id}::${importer}`;
            if (!seen.has(key)) {
              result = check(
                pluginOptions,
                resolved.id,
