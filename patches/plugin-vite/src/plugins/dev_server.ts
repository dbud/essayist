...
import { ASSET_CACHE_BUST_KEY } from "jsr:@fresh/core@^2.0.0/internal";
import * as path from "jsr:@std/path@1";
import {
  createRequest,
  sendResponse,
} from "npm:@remix-run/node-fetch-server@^0.12.0";
import type { DevEnvironment, Plugin } from "npm:vite@^8.3.0";
...
export function devServer(freshConfig: ResolvedFreshViteConfig): Plugin[] {
  ...
  return [
    ...
    },
    {
      ...
      hotUpdate(options) {
        const clientMod =
          options.server.environments.client.moduleGraph.getModulesByFile(
            options.file,
          );
        ...
        const ssrMod =
          options.server.environments.ssr.moduleGraph.getModulesByFile(
            options.file,
          );
        if (ssrMod !== undefined) {
...
async function collectCss(id: string, env: DevEnvironment) {
  ...
  while ((current = queue.pop()) !== undefined) {
    ...
    if (mod === undefined || mod.transformResult === null) {
      ...
      mod =
        env.moduleGraph.idToModuleMap.get(current) ??
    ...
    if (
      (current.endsWith(".scss") || current.endsWith(".css")) &&
      mod.transformResult
    ) {
      ...
      if (match !== null) {
        ...
        out.push(
          `<style type="text/css" vite-module-id="${hashCode(
            id,
          )}">${content}</style>`,
        );
      }
