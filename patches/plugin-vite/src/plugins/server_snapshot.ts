        ...
        async handler() {
          ...
          if (isDev && server !== undefined) {
            ...
            for (const chunk of Object.values(manifest)) {
              ...
              if (chunk.name?.startsWith("fresh-island__")) {
                ...
                if (idx > -1 && chunk.src) {
                  const src = chunk.src
                    .slice(idx)
                    .replace(/(https?):\/([^/])/, (_m, protocol, rest) => {
            ...
            for (const dir of options.staticDir) {
              ...
              const entries = await fsAdapter.walk(dir, {
            ...
            if (await fsAdapter.isDirectory(clientOutDir)) {
              ...
              const clientFiles = await fsAdapter.walk(clientOutDir, {
                followSymlinks: false,
                includeDirs: false,
                includeFiles: true,
              });
              ...
              for await (const entry of clientFiles) {
                const relative = path
                  .relative(clientOutDir, entry.path)
                  .replaceAll("\\", "/");
