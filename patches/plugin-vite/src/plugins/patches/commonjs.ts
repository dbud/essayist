...
import type { NodePath, PluginObj, types } from "npm:@babel/core@^7.28.0";

const BUILTINS = new Set(builtinModules);

export function cjsPlugin({ types: t }: { types: typeof types }): PluginObj {
      ...
      Program: {
        ...
        exit(path, state) {
          ...
          for (const name of exported) {
            ...
            idExports.push(t.exportSpecifier(id, t.identifier(name)));
          }
...
function getExportsAssignName(
  ...
  if (
    ((t.isMemberExpression(node.object) && isModuleExports(t, node.object)) ||
...
function isEsModuleFlag(
  ...
  return (
    ((t.isMemberExpression(left.object) && isModuleExports(t, left.object)) ||
