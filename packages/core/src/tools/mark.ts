...
const inputSchema = z.object({
  ...
  selected_text: z.string().describe(
  ...
  comment: z.string().describe(
    "The comment or note to attach to this mark.",
  ),
...
const outputSchema = z.object({
  mark_id: z.string().describe(
    "Unique ID of the created mark.",
  ),
  thread_id: z.string().describe(
    "Stable thread ID for tracking this mark across versions.",
  ),
  marked: z.boolean().describe(
...
export function createMarkTool(vfs: VFS): ToolPrompt {
  return {
    ...
    tool: tool({
      name: "mark",
      description: "Place a mark (annotation) on a text span in a file. " +
        "Returns a mark_id and thread_id. " +
        "If selected_text appears multiple times, use line_hint to specify which occurrence.",
      inputSchema,
      outputSchema,
      execute: async (
        { path, selected_text, comment, label, line_hint },
      ): Promise<MarkOutput> => {
        return await vfs.mark(
          path,
          selected_text,
          comment,
          { label, lineHint: line_hint },
        );
      },
    }),
  };
