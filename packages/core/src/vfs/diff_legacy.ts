// Legacy LCS-DP diff implementation, copied verbatim from main (commit
// fcc35fd) for A/B benchmarking against the new Myers implementation in
// diff.ts. Kept as a sibling module so the bench can import both in one
// process. Not exported from the package; delete after benchmarking.

import { createTokenizer, type Token } from "@/vfs/text_utils.ts";

export interface DiffHunkLegacy {
...
/**
 * Token-level diff via the O(N*M) LCS dynamic-programming table.
 */
export function computeDiffLegacy(
  oldText: string,
  newText: string,
): DiffHunkLegacy[] {
...
function lcsDiff(oldTokens: Token[], newTokens: Token[]): DiffOp[] {
