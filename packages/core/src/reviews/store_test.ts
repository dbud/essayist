...
Deno.test("ReviewStore -- completeRun sets status, summary, completedAt", async () => {
  ...
  assertEquals(
    (await store.getRun({ wsId: "ws", id: run.id }))?.status,
    "completed",
  );
});
...
Deno.test("ReviewStore -- completeRun/failRun unknown id return undefined", async () => {
  const store = createStore();
  assertEquals(
    await store.completeRun({ wsId: "ws", id: "nope", summary: "x" }),
    undefined,
  );
  assertEquals(
    await store.failRun({ wsId: "ws", id: "nope", error: "x" }),
    undefined,
  );
});
...
Deno.test("ReviewStore -- listRuns newest first", async () => {
  ...
  const runs = await store.listRuns({ wsId: "ws" });
  assertEquals(runs.length, 2);
  assertEquals(runs[0].id, b.id);
  assertEquals(runs[1].id, a.id);
});

Deno.test("ReviewStore -- listRuns filters by path", async () => {
  const store = createStore();
  const a = await store.createRun({
    wsId: "ws",
    path: "a.txt",
  ...
  const aRuns = await store.listRuns({ wsId: "ws", path: "a.txt" });
  assertEquals(aRuns.length, 2);
  assertEquals(aRuns[0].id, a2.id);
  assertEquals(aRuns[1].id, a.id);

  const bRuns = await store.listRuns({ wsId: "ws", path: "b.txt" });
...
Deno.test("ReviewStore -- listRuns is workspace-scoped", async () => {
  ...
  assertEquals((await store.listRuns({ wsId: "ws1" })).length, 1);
  assertEquals((await store.listRuns({ wsId: "ws2" })).length, 1);
});
